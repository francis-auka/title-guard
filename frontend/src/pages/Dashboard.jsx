...
function Dashboard() {
    ...
    const [settingsForm, setSettingsForm] = useState({ nationalId: "", phone: "" });
    ...
    const handleSettingsSave = useCallback(async (e) => {
        e.preventDefault();
        ...
        try {
            ...
            setSettingsError(err.response?.data?.message || "Update failed. Please try again.");
        } finally {
            setSettingsSaving(false);
        }
    }, [settingsForm, refreshUser]);
        ...
        <div className="page-wrapper py-10">
            <div className="container-wide">
                ...
                <div className="card mb-8 overflow-hidden">
                    <button
                        id="account-settings-toggle"
                        onClick={() => { setSettingsOpen(o => !o); setSettingsError(""); setSettingsSuccess(""); }}
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-500">
                                    National ID: <span className={user?.nationalId ? "text-slate-300" : "text-amber-500/80 italic"}>{user?.nationalId || "Not set"}</span>
                                    <span className="mx-2 text-slate-700">·</span>
                                    Phone: <span className={user?.phone ? "text-slate-300" : "text-amber-500/80 italic"}>{user?.phone || "Not set"}</span>
                                </p>
                            </div>
                        </div>
                        <svg
                    ...
                    {settingsOpen && (
                        <div className="px-5 pb-6 pt-1 border-t border-slate-700/50">
                            {settingsError && (
                                <div className="mb-4 mt-4 px-3 py-2.5 rounded-lg bg-red-500/10 border border-red-500/30 text-red-400 text-sm flex items-center gap-2">
                                    <span>⚠️</span> {settingsError}
                                </div>
                            )}
                            {settingsSuccess && (
                                <div className="mb-4 mt-4 px-3 py-2.5 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-sm flex items-center gap-2">
                                    <span>✅</span> {settingsSuccess}
                                </div>
                            )}

                            <form onSubmit={handleSettingsSave} className="mt-4">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                                    <div className="form-group">
                                        <label htmlFor="settings-nationalId" className="label">
                                            National ID
                                        </label>
                                        <input
                                            id="settings-nationalId"
                                            type="text"
                                            value={settingsForm.nationalId}
                                            onChange={(e) => setSettingsForm(f => ({ ...f, nationalId: e.target.value }))}
                                ...
                                <p className="text-[11px] text-slate-500 mb-4 flex items-center gap-1.5">
                                    ...
                                    Your phone number is used to send SMS notifications after deed registration, verification, and M-Pesa payments.
                                </p>
                                <div className="flex items-center gap-3">
                                    <button
                                    ...
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            setSettingsForm({ nationalId: user?.nationalId || "", phone: user?.phone || "" });
                                            setSettingsError("");
                                            setSettingsSuccess("");
                                        }}
